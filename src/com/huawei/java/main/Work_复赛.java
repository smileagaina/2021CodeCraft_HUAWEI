...
public class Work_复赛 {
    ...
    // expansion参数
    private static double expansion_para = 1.15;
    // 服务器信息映射
...
//    性价比
//    static Comparator<Server> cmp = Comparator.comparingDouble(s -> (s.oricost+s.dailycost)*1.0/(s.cpu + s.memory));
...
//            return (int)((Math.min(o1.cpusizel,o1.cpusizer)+Math.min(o1.memsizel,o1.memsizer))-(Math.min(o2.cpusizel,o2.cpusizer)+Math.min(o2.memsizel,o2.memsizer)));
    ...
    // 文件路径
    private static String filePath = "src/com/huawei/resource/复赛/training-";
    // 总费用
    ...
    public static void expansion_new(Virtual curVirtual, Request request, List<Server> serverListSelected) {
        ...
        if(curVirtual.isBi==1){
            ...
            for (Server server : serverListSelected) {
                if ((server.cpu*1.0 >= curVirtual.cpu*expansion_para) && (server.memory*1.0 >= curVirtual.memory*expansion_para) &&
            ...
            // *** 堆顶找不到，然后找下一个，所以需要循环，直接取堆顶不行
            for (Server server : serverListSelected) {
                if ((server.cpu/2.0*1.0 >= curVirtual.cpu*expansion_para) && (server.memory/2.0*1.0 >= curVirtual.memory*expansion_para) &&
