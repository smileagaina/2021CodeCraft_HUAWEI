...
//    private static final String filePath = "src/com/huawei/resource/test_data.txt";
    private static final String filePath = "src/com/huawei/resource/复赛/training-1.txt";
    // 总费用
    ...
    private static int requestDays = 0;
...
//                    continue;
            ...
            int requestDays = Integer.parseInt(line[0]);
            ...
            for(int i=requestDays-K;i<requestLists.size();i++){
                process1(i);
                System.out.println(i+"===========");
            }
            //输出第i天的日志
...
//                continue;
    ...
    public static void process1(int i){
        //处理第i天的请求
        migrate5(i);
        List<Request> list = requestLists.get(i);
...
//            Collections.sort(list,cmpwlh);
        ...
        // 此时所有的请求都已经读入，执行剩余天数的请求
        for (int k=requestDays-K;k<requestLists.size();k++) {
            // 迁移策略
//            migrate5(k);
